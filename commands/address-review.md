...
4. Use the `todowrite` tool or create a plan in `tasks/` listing all suggestions.
5. For each point:
   - Identify the affected code.
   - Implement the change using `edit` or `write`.
   - Run relevant tests and linting.
6. Once all points are addressed, create `git commit --fixup <commit_hash>` for the relevant original commits.
7. Verify everything passes with `make tidy` or equivalent project standards.
